...
% 求出节点的平均度
 ...
 for i=1:50 %实验50次
     ...
     for falselinkpercent=0.0:0.1:1.0
         %--一半热门一半随机
         train=RebaseTrain(train,k,0.5, 0 ,net,falselinkpercent);
%          disp('CN...1');
         cnauc=CN(train,test);
         results(j,i,1)=cnauc;
...
%          disp('TS...32');
     ...
     disp(i);
      ...
      %---把原始实验数据也写入xls
     for i = 1:32
         outfile = strcat('out/CE/falselinkhot/Figori_algorithm_',int2str(i),'.xlsx');
%         outfile = strcat('out/test/Figori_algorithm_',int2str(i),'.xlsx');
    ...
    %      把结果输出到文件中去
    outfile = strcat('out/res2/CEhotrandom_avg','.xlsx');
